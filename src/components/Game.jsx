  ...
  useEffect(() => {
    ...
    async function getData() {
      ...
      const normalizedData = await data.results.map((piece) => {
        const sortedAns = sortAnswers([
          ...piece.incorrect_answers,
          piece.correct_answer,
        ]);
    ...
    <div className="game">
      ...
      {isFinished ? (
        <div className="game--score">
          <span className="game--score--text">
            You scored {score}/5 correct answers.
          </span>
          <button
            onClick={resetGame}
            className="primary-btn game--score--button"
          >
            Play again
          </button>
        </div>
        ...
        <button
          className="primary-btn game--check-answers"
